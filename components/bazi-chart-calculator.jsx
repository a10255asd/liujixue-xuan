...
import { baZiArchiveOptions, baZiExampleInputs, baZiGenderOptions, baZiSectOptions, baZiYunSectOptions, calculateBaZiChart, defaultBaZiInput } from '@/lib/bazi-chart'
import {
  getAreaOptions,
...
function ArchiveTextField({ id, label, value, onChange, placeholder, textarea = false, maxLength = 120 }) {
  ...
  return (
    <div className='chart-field wide'>
      <label htmlFor={`bazi-${id}`}>{label}</label>
      {textarea
  ...
  return (
    <section className='chart-section-card bazi-workflow-card'>
      <div className='chart-section-head'>
        ...
        <span className='chart-source'>记录 / 校时 / 导出</span>
      </div>
...
function BaZiReviewCard({ result }) {
  const rows = buildBaZiReviewRows(result)
  ...
  return (
    <section className='chart-section-card'>
      <div className='chart-section-head'>
        ...
        <span className='chart-source'>来源 / 校时 / 边界</span>
      </div>
