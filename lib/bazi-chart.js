...
export const baZiArchiveProfiles = {
  archive: {
    label: '出生档案',
    description: '固定资料留存',
    review: '先固定出生证明、户口本或家人记录中的原始说法，再核对出生地、真太阳时和子时口径。',
    action: '把本次排盘图片和文本下载留档，后续只追加新证据和来源。',
    observe: '复盘时优先查看资料来源是否变化，不把排盘字段当作人生判断。'
  },
  calibration: {
    label: '校时复核',
    description: '时间不确定',
    review: '把候选时段、来源可信度和跨时辰边界写清楚，再用相邻时辰逐一留档。',
    action: '同步打开出生校时或寻时定盘工具，逐个候选时间保存结果。',
    observe: '不要只凭盘面反推时辰，优先核对出生证、医院记录和家人说法。'
  },
  study: {
    label: '术语学习',
    description: '学习字段口径',
    review: '把四柱、十神、藏干、纳音和神煞当作术语样本，逐项核对字段来源。',
    action: '先查看术语说明和干支五行资料，再保存一份可复读的字段记录。',
    observe: '学习时只比较字段和口径，不把单个术语延伸成吉凶结论。'
  },
  comparison: {
    label: '方案对照',
    description: '多盘横向比较',
    review: '用于同一人不同出生时间、不同子时口径或不同起运算法的横向对照。',
    action: '为每个候选方案分别下载图片和文本，并在文件名或备注里写清口径。',
    observe: '比较时只看哪些字段变化，暂不直接筛选所谓唯一正确结果。'
  }
}

export const baZiArchiveOptions = Object.entries(baZiArchiveProfiles).map(([value, profile]) => ({
  value,
  label: profile.label,
...
export const baZiExampleInputs = [
  {
    ...
    input: {
      ...
      calibrationNotes: '接近晚子时边界，需对照子时换日口径。',
      timeMode: 'trueSolar',
      yunSect: 2,
      sect: 1
    }
  },
  {
    ...
    input: {
      ...
      birthLatitude: 29.79,
      archivePurpose: 'study',
      birthSource: '样例资料',
      calibrationNotes: '午时样例，用于核对真太阳时和神煞字段。',
      timeMode: 'trueSolar',
      yunSect: 2,
      sect: 2
    }
...
export function normalizeBirthInput(input = {}) {
  ...
  const archivePurpose = baZiArchiveProfiles[merged.archivePurpose] ? merged.archivePurpose : defaultBaZiInput.archivePurpose
  const archivePurposeLabel = baZiArchiveProfiles[archivePurpose].label
  const birthSource = normalizeArchiveText(merged.birthSource, 120, defaultBaZiInput.birthSource)
  const calibrationNotes = normalizeArchiveText(merged.calibrationNotes, 180)

  return { year, month, day, hour, minute, gender, birthProvinceCode, birthCityCode, birthAreaCode, birthPlace, birthLongitude, birthLatitude, archivePurpose, archivePurposeLabel, birthSource, calibrationNotes, timeMode, yunSect, sect }
}
...
export function calculateBaZiChart(input = {}) {
  ...
  return {
    ...
    notes: [
      ...
      '性别字段只用于大运顺逆和起运序列。',
      '档案字段：用途、资料来源和校时线索只用于留档复核，不参与四柱计算。',
      '神煞口径：按公开神煞大全和常见排盘软件规则输出字段集合，不输出吉凶断语。',
      '输出范围：只展示出生档案、排盘字段和复核清单。'
    ]
  }
}
