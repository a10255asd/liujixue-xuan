...
const promptReviewProfiles = {
  bazi: {
    title: '八字字段核验',
    checklist: [
      '公历/农历、出生地、性别、真太阳时或标准时口径',
      '四柱年柱/月柱/日柱/时柱、日主、胎元/命宫等扩展字段',
      '十神、藏干、纳音、空亡、神煞、大运、流年是否齐全',
      '晚子时换日、起运折算、节气分月等口径是否已经注明'
    ],
    riskNotes: [
      '先核验出生时间和时区/经度修正，不要自行改盘',
      '神煞、起运和子时口径不同软件可能不同，需标注来源差异',
      '只整理结构要点和待问清单，不直接给人生结论'
    ],
    outputSteps: [
      '字段完整度表',
      '口径/缺失/冲突清单',
  ...
  ziwei: {
    ...
    checklist: [
      '公历/农历、出生地、性别、真太阳时或标准时口径',
      '命宫、身宫、五行局、命主、身主、十二宫宫位',
      '主星、辅星、煞曜、四化、宫干、流年/大限字段是否齐全',
      '闰月、晚子时、排盘流派和宫位顺序是否已经注明'
    ],
    riskNotes: [
      '先核验命盘基础信息，不要自行补星曜或重排宫位',
      '不同流派四化、闰月和晚子时处理可能不同，需保留差异说明',
      '只做宫位/星曜字段整理，不直接输出命运定论'
    ],
    outputSteps: [
      '命盘基础字段表',
      '十二宫字段完整度',
  ...
  liuyao: {
    title: '六爻字段核验',
    checklist: [
      '事项、起卦方式、起卦时间、干支、月建、日辰、旬空',
      '本卦、变卦、动爻、变爻、世爻、应爻是否齐全',
      '六亲、六神、地支纳甲、五行、伏神/飞神字段是否齐全',
      '合、冲、刑、害、墓、绝、生、克、暗动、进退神等只按已提供字段整理'
    ],
    riskNotes: [
      '先核验事项背景和起卦方式，不要擅自更换用神或补卦',
      '用神、旺衰、应期属于解释层，只能作为待核验问题列出',
      '不直接断成败、复合、财运、疾病或具体应期'
    ],
    outputSteps: [
      '起卦信息完整度表',
      '卦身/世应/动变/六亲六神字段摘要',
...
const getPromptReviewProfile = chartType => promptReviewProfiles[chartType] || promptReviewProfiles.other
...
const buildAnalysisPrompt = input => {
  ...
  const profile = getPromptReviewProfile(input.chartType)
  ...
  return [
    ...
    '',
    `${profile.title}：`,
    ...profile.checklist.map((item, index) => `${index + 1}. ${item}`),
    '',
    '常见误差点：',
    ...profile.riskNotes.map(item => `- ${item}`),
    '',
    '排盘字段：',
    chartText,
    '',
    `输出格式：${outputName}`,
    '输出要求：',
    ...profile.outputSteps.map(item => `- ${item}`),
    '- 所有观察都要回指到原始字段；字段里没有的内容请标为“未提供”。',
...
const calculateAiPrompt = input => {
  ...
  const context = cleanPromptInput(input.context, '无额外背景。')
  const chartText = cleanPromptInput(input.chartText, '请在这里粘贴完整排盘字段。')
  const chartLineCount = compactTextLines(input.chartText).length
  const prompt = buildAnalysisPrompt({ ...input, question, context, chartText })

  return result({
    title: 'AI 解析提示词',
    subtitle: `${toolName} · ${modeName}`,
    badges: [toolName, modeName, outputName, chartLineCount ? `${chartLineCount} 行字段` : '待粘贴字段'],
    copyText: prompt,
    ...
    sections: [
      {
        title: '交接摘要',
        rows: buildRows([
          ['材料状态', chartLineCount ? `已粘贴 ${chartLineCount} 行排盘字段，可以生成接力提示词。` : '还没有粘贴排盘字段，建议先从八字、紫微、六爻或记录中心导入。'],
          ['推荐下一步', chartLineCount ? '复制提示词交给 AI，让它先做字段核验、结构化摘要和待补充问题清单。' : '先补完整排盘字段，再复制提示词。'],
          ['输出边界', '只做字段整理、核验和继续追问，不输出吉凶断语、应期承诺或重大决策建议。']
        ])
      },
      {
        title: '提示词参数',
      ...
      },
      {
        title: profile.title,
        rows: buildRows([
          ['必须核验', profile.checklist.join(' / ')],
          ['常见误差点', profile.riskNotes.join(' / ')],
          ['输出步骤', profile.outputSteps.join(' / ')]
        ])
      },
      {
        title: '边界要求',
...
const inferPromptChartType = value => {
  ...
  if (text.includes('梦')) return 'dream'
  if (text.includes('黄历') || text.includes('择日') || text.includes('运势')) return 'calendar'
  return 'other'
}

const inferPromptMode = value => {
  const text = String(value || '')
  if (/六爻|梅花|奇门|六壬|每日一卦|本卦|变卦|动爻|世应/.test(text)) return 'questions'
  if (/八字|紫微|四柱|命宫|身宫/.test(text)) return 'verify'
  return 'structure'
}

const buildHandoffQuestion = chartType => {
  if (chartType === 'bazi') return '请按八字字段核验清单，先检查出生时间、真太阳时/标准时、四柱、十神、大运流年和神煞字段完整度，再整理待补充问题。'
  if (chartType === 'ziwei') return '请按紫微字段核验清单，先检查命宫、身宫、十二宫、主星辅星、四化、大限流年和时间口径，再整理待补充问题。'

  if (['liuyao', 'meihua', 'qimen', 'daliuren'].includes(chartType)) {
    return '请先核验起卦字段、事项背景和可能缺失项，再列出适合继续追问的问题清单。'
  }

  if (chartType === 'calendar') return '请先核验日课/择日字段，再整理时间口径、适用场景和待补充信息。'
  if (chartType === 'dream') return '请先按事实、情绪、意象和现实背景整理这份记录，再列出可继续追问的问题。'
  return '请先核验字段完整度，再整理这份排盘里可以继续追问的结构要点。'
}

const applyAiPromptHandoff = (form, handoff) => {
  const sourceText = `${handoff.sourceTool || ''} ${handoff.sourceTitle || ''} ${handoff.text || ''}`
  const chartType = inferPromptChartType(sourceText)

  return {
    ...form,
    chartType,
    mode: inferPromptMode(sourceText),
    question: buildHandoffQuestion(chartType),
    context: appendSourceContext(form.context, sourceLabel(handoff)),
...
const compatibilityReviewProfiles = {
  bazi: {
    title: '八字合盘核验清单',
    perPerson: [
      '对象 A 和对象 B 先分别核验出生时间、出生地、性别、真太阳时或标准时口径',
      '分别核验四柱、日主、十神、藏干、纳音、空亡、神煞、大运/流年字段',
      '标注晚子时、节气分月、起运折算和神煞口径是否已经说明'
    ],
    comparison: [
      '只把同类字段并排，例如四柱对四柱、十神对十神、五行结构对五行结构',
      '把共同点、差异点、缺失项分别列出，不把差异写成关系结果',
      '如果两份盘的排盘口径不同，先暂停对照并列出需要补充的口径'
    ],
    riskNotes: [
      '不要自行改盘、补四柱或替用户选择流派',
      '合盘阶段只整理字段，不输出关系好坏、婚恋成败或应期'
    ]
  },
  ziwei: {
    title: '紫微合盘核验清单',
    perPerson: [
      '对象 A 和对象 B 先分别核验出生时间、出生地、性别、真太阳时或标准时口径',
      '分别核验命宫、身宫、五行局、命主、身主、十二宫、主星辅星和四化',
      '标注闰月、晚子时、排盘流派、宫干、大限/流年字段是否已经说明'
    ],
    comparison: [
      '只把同类宫位和同类星曜字段并排，不跨体系混写',
      '先列命身宫、三方四正、四化和宫位字段的共同点/差异点',
      '缺少宫位、星曜或大限字段时，先列待补项再继续'
    ],
    riskNotes: [
      '不要自行补星曜、重排宫位或替用户确定流派',
      '只输出字段对照和待核验点，不输出宿命式关系结论'
    ]
  },
  mixed: {
    title: '混合资料合盘核验清单',
    perPerson: [
      '先确认两份资料分别来自八字、紫微或其他排盘体系',
      '每个对象先在自己的体系内核验基础字段，不把八字字段写成紫微字段',
      '标注出生信息、时间口径、地点、流派和资料来源是否齐全'
    ],
    comparison: [
      '只在同一主题层面做交叉观察，例如时间口径、字段完整度、问题背景',
      '八字和紫微只能并列摘要，不能直接逐项等价替换',
      '不同体系指向不一致时，只记录差异和待核验字段'
    ],
    riskNotes: [
      '混合资料更容易口径错位，先做来源标注再做整理',
      '不要把不同体系硬合成单一关系结论'
    ]
  },
  other: {
    title: '通用合盘核验清单',
    perPerson: [
      '先确认对象 A 和对象 B 的资料来源、时间、地点、事项背景',
      '分别列出每份资料的核心字段、缺失字段和不确定口径',
      '字段不足时先要求补充，不补写未提供信息'
    ],
    comparison: [
      '只做同类字段并排和材料完整度检查',
...
const fieldCountText = count => count ? `${count} 行字段` : '未填写'

const appendSourceContext = (context, source) => {
  const current = String(context || '').trim()
  const line = `来自记录中心：${source}`
  if (!current) return line
  if (current.includes(line)) return current
  return `${current}\n${line}`
}

const getCompatibilityReviewProfile = chartType => compatibilityReviewProfiles[chartType] || compatibilityReviewProfiles.other

const buildCompatibilityProfileText = profile => [
  `${profile.title}：`,
  '对象独立核验：',
  ...profile.perPerson.map((item, index) => `${index + 1}. ${item}`),
  '',
  '并排对照方式：',
  ...profile.comparison.map((item, index) => `${index + 1}. ${item}`),
  '',
  '常见误差点：',
  ...profile.riskNotes.map(item => `- ${item}`)
].join('\n')
...
const buildCompatibilityPrompt = input => {
  ...
  const profile = getCompatibilityReviewProfile(input.chartType)
  ...
  return [
    ...
    `背景补充：${context}`,
    '',
    buildCompatibilityProfileText(profile),
    '',
    `${personA} 排盘字段：`,
    chartA,
    '',
    ...
    '',
    '输出要求：',
    '- 先分别核验对象 A 和对象 B 的资料来源、字段完整度和口径。',
...
const calculateCompatibility = input => {
  ...
  const readyCount = [chartALines.length, chartBLines.length].filter(Boolean).length
  const missing = [
  ...
  return result({
    ...
    badges: [chartType, focus, `${readyCount}/2 份资料`, relation],
    copyText: prompt,
    ...
    sections: [
      {
        title: '交接摘要',
        rows: buildRows([
          ['资料状态', `${personA}：${fieldCountText(chartALines.length)}；${personB}：${fieldCountText(chartBLines.length)}。`],
          ['推荐下一步', missing.length ? `先补充 ${missing.join('、')}，再复制对照提示词。` : '两份字段已就位，可以复制对照提示词做并排核验和继续追问。'],
          ['输出边界', '只做字段并排、结构梳理和待核对点，不输出关系好坏、成败判断或结果保证。']
        ])
      },
      {
        title: '对照设置',
      ...
      },
      {
        title: profile.title,
        rows: buildRows([
          ['对象独立核验', profile.perPerson.join(' / ')],
          ['并排对照方式', profile.comparison.join(' / ')],
          ['常见误差点', profile.riskNotes.join(' / ')]
        ])
      },
      {
...
const synthesisMaterialProfiles = {
  birthChart: {
    title: '出生盘材料核验',
    checklist: [
      '先确认八字/紫微/校时/姓名等材料来源，不把不同体系字段混写',
      '核验公历/农历、出生地、性别、真太阳时或标准时口径是否齐全',
      '八字重点看四柱、日主、十神、藏干、纳音、神煞、大运流年字段',
      '紫微重点看命宫、身宫、五行局、十二宫、主辅星、四化、大限流年字段'
    ],
    riskNotes: [
      '晚子时、闰月、节气分月、起运折算和真太阳时可能造成差异',
      '只按原始字段整理，不自行改盘、补星曜或重算四柱'
    ]
  },
  questionChart: {
    title: '问事盘材料核验',
    checklist: [
      '先确认事项、起盘方式、起盘时间、提问背景和是否一事一问',
      '六爻重点看本卦、变卦、动爻、世应、六亲、六神、纳甲、伏神飞神',
      '梅花、奇门、六壬重点保留起局/起课字段、宫位/盘面和空亡等基础字段',
      '用神、旺衰、应期、成败只列为后续分析问题，不在合参阶段定论'
    ],
    riskNotes: [
      '事项背景不足时不要替用户补目的',
      '不同问事体系不能互相替代或强行合成一个判断'
    ]
  },
  tarotText: {
    title: '塔罗材料核验',
    checklist: [
      '先确认牌阵、问题、抽牌时间、每张牌的位置和正逆位',
      '只复述牌名、位置、关键词和用户背景，不把牌义写成排盘结论',
      '如果与八字/紫微/六爻同场合参，只作为心理/叙事材料旁证',
      '缺少牌阵或问题时，先要求补充再做整理'
    ],
    riskNotes: [
      '塔罗材料不能替代出生盘或问事盘字段',
  ...
  calendarText: {
    title: '日课/择日材料核验',
    checklist: [
      '先确认日期范围、候选日期、时辰、节气、干支和适用事项',
      '区分黄历字段、节气字段、时辰字段和用户现实约束',
      '只整理候选时间条件，不承诺某日会带来指定结果',
      '如果涉及上线、搬家、签约等事项，先列现实限制和待确认条件'
    ],
    riskNotes: [
      '日课字段不应覆盖用户现实安全、合同、预算或执行条件',
      '不同历法/时区口径未注明时，先标记为待核验'
    ]
  },
  notes: {
    title: '补充材料整理',
    checklist: [
      '提取用户目标、现实背景、限制条件、时间线和已经发生的事实',
      '把主观感受、事实描述、猜测和诉求分开记录',
      '指出哪些问题需要用户补充，而不是用玄学字段代替事实',
      '最后沉淀成下一轮 AI 或人工老师可直接接手的摘要'
    ],
    riskNotes: [
      '补充材料只能解释用户语境，不能当作排盘字段',
      '涉及医疗、法律、投资、重大人生决策时保持边界'
    ]
  }
}
...
const buildSynthesisMaterialInstructions = input => Object.entries(synthesisMaterialProfiles)
  .map(([key, profile]) => {
    const lineCount = compactTextLines(input[key]).length
    const state = lineCount ? `已提供 ${lineCount} 行` : '未提供，记录为缺失，不要补写'

    return [
      `${profile.title}（${state}）：`,
      ...profile.checklist.map((item, index) => `${index + 1}. ${item}`),
      `常见误差：${profile.riskNotes.join('；')}`
    ].join('\n')
  })
  .join('\n\n')

const synthesisMaterialRows = input => Object.entries(synthesisMaterialProfiles).map(([key, profile]) => {
  const lineCount = compactTextLines(input[key]).length
  const state = lineCount ? `已提供 ${lineCount} 行` : '未提供'
  return [
    profile.title,
    `${state}：${profile.checklist.join(' / ')}`
  ]
})
...
const buildSynthesisPrompt = input => {
  ...
  return [
    ...
    '',
    '分材料核验清单：',
    buildSynthesisMaterialInstructions(input),
    '',
    '出生盘字段：',
    birthChart,
    '',
    ...
    '',
    '合参处理顺序：',
    '- 先按材料类型单独核验完整度、口径和缺失项。',
    '- 再围绕用户主题提取可交叉观察的同一类问题。',
    '- 如果不同体系指向不一致，只记录差异、可能原因和待核验字段。',
    '- 不把塔罗、梦境、日课或补充材料写成出生盘/问事盘结论。',
    '',
    '输出要求：',
    '- 先列材料清单和完整度，指出缺失字段。',
...
const calculateSynthesis = input => {
  ...
  const missingLabels = lineCounts.filter(([, count]) => count === 0).map(([label]) => label)
  ...
  return result({
    ...
    badges: [focus, `${presentCount}/5 类材料`, presentCount ? '可接力' : '待填材料'],
    copyText: prompt,
    ...
    sections: [
      {
        title: '交接摘要',
        rows: buildRows([
          ['材料状态', lineCounts.map(([label, count]) => `${label}${count ? count : 0}行`).join(' / ')],
          ['推荐下一步', presentCount ? '复制合参提示词，让 AI 先列材料清单、字段冲突和可继续追问的问题。' : '先从记录中心或各工具导入至少一类材料。'],
          ['待补充', missingLabels.length ? missingLabels.join('、') : '五类材料均已填写，仍建议人工核对时间、地点和流派口径。']
        ])
      },
      {
        title: '合参设置',
      ...
      },
      {
        ...
        rows: buildRows([
          ...
          ['待补充', missingLabels.join('、') || '五类材料均已填写，仍建议人工核对口径。']
        ])
      },
      {
        title: '分材料核验清单',
        rows: buildRows(synthesisMaterialRows(input))
      },
      {
        title: '材料摘要',
...
const buildDreamPrompt = input => {
  ...
  return [
    `你将基于一份「梦境记录」做「${focus}」整理。`,
    '',
    '请严格遵守：',
    '1. 只基于我提供的梦境内容、醒来情绪和现实背景整理，不要编造梦境细节。',
    '2. 不输出恐吓式、绝对化、宿命论表达，不做吉凶、预兆、应期或结果保证。',
    '3. 不给医疗、法律、投资和人生重大决策建议；如涉及强烈持续困扰，只建议寻求现实支持或专业帮助。',
    '4. 明确区分「梦境事实复述」「情绪/意象观察」「可能的现实关联」「待补充问题」。',
    '5. 以辅助记录和继续提问为目标，不把推测说成确定结论。',
    '',
    `梦境标题：${title}`,
    `关注方向：${focus}`,
    `醒来情绪：${mood}`,
    ...
    '- 先按时间线整理梦境片段。',
    '- 再列出人物、场景、情绪和意象清单。',
    '- 然后给出保守的观察方向和可继续追问的问题。',
    '- 最后输出一份适合继续保存或交给其他 AI 接手的摘要。'
  ].join('\n')
}
...
const applyCompatibilityHandoff = (form, handoff) => {
  ...
  return {
    ...form,
    chartType: inferCompatibilityChartType(`${handoff.sourceTool || ''} ${handoff.sourceTitle || ''} ${handoff.text || ''}`),
...
export const structuredTools = {
  ...
  synthesis: {
    ...
    defaultInput: {
      topic: '当前事项多工具材料整理',
      focus: 'overview',
      question: '请先列材料完整度，再按工具来源整理关键字段，最后给出可继续追问的问题清单。',
      context: '只需要结构化整理，不需要结论判断。',
      birthChart: '',
      questionChart: '',
      tarotText: '',
      calendarText: '',
    ...
    fields: [
      { key: 'topic', label: '主题', type: 'text', placeholder: '例如：合作事项、关系沟通、择日上线' },
      { key: 'focus', label: '关注方向', type: 'select', options: promptOptions(synthesisFocusLabels), help: '决定提示词的组织重点，不会生成最终判断。' },
      { key: 'question', label: '具体问题', type: 'textarea', placeholder: '写清楚这次希望 AI 帮你整理什么。' },
      { key: 'context', label: '背景补充', type: 'textarea', placeholder: '只放现实背景、已知限制、需要避开的判断口径。' },
      { key: 'birthChart', label: '出生盘字段', type: 'textarea', placeholder: '粘贴八字、紫微、姓名、校时等出生盘字段。' },
      { key: 'questionChart', label: '问事盘字段', type: 'textarea', placeholder: '粘贴六爻、梅花、奇门、大六壬等问事盘字段。' },
      { key: 'tarotText', label: '塔罗字段', type: 'textarea', placeholder: '粘贴牌阵、位置、牌面、正逆位和关键词。' },
      { key: 'calendarText', label: '日课/择日字段', type: 'textarea', placeholder: '粘贴黄历、节气、择日、时辰等时间字段。' },
      { key: 'notes', label: '补充材料', type: 'textarea', placeholder: '放聊天记录摘要、现实背景、限制条件或人工备注。' }
    ],
    recordSlots: [
      { key: 'birthChart', label: '作为出生盘', indicatorLabel: '出生盘字段' },
      { key: 'questionChart', label: '作为问事盘', indicatorLabel: '问事盘字段' },
      { key: 'tarotText', label: '作为塔罗', indicatorLabel: '塔罗字段' },
      { key: 'calendarText', label: '作为日课', indicatorLabel: '日课字段' },
      { key: 'notes', label: '作为补充', indicatorLabel: '补充材料' }
    ],
  ...
  aiPrompt: {
    ...
    defaultInput: {
      ...
      mode: 'structure',
      outputFormat: 'table',
      question: '请先核验字段完整度，再整理这份排盘里可以继续追问的结构要点。',
      context: '只做字段复述、结构化观察和待核验点，不做吉凶断语。',
      chartText: ''
    },
    fields: [
      { key: 'chartType', label: '排盘类型', type: 'select', options: promptOptions(promptToolLabels), help: '从记录中心跳转时会自动识别。' },
      ...
      { key: 'question', label: '具体问题', type: 'textarea', placeholder: '明确要 AI 做核验、摘要、问题清单，还是简版提纲。' },
      { key: 'context', label: '背景补充', type: 'textarea', placeholder: '补现实背景、已知口径、需要避免的结论类型。' },
      { key: 'chartText', label: '排盘字段', type: 'textarea', placeholder: '粘贴完整排盘字段，或从记录中心/工具页直接送入。' }
    ],
  ...
  compatibility: {
    ...
    defaultInput: {
      ...
      personB: '对象 B',
      relation: '未说明关系',
      question: '请先核对两份资料字段完整度，再并排整理可对照字段和待补充信息。',
      context: '只做字段对照，不输出关系好坏或结果判断。',
      chartA: '',
      chartB: ''
    },
    fields: [
      { key: 'chartType', label: '排盘类型', type: 'select', options: promptOptions(compatibilityChartLabels), help: '两份资料可同为八字/紫微，也可混合。' },
      { key: 'focus', label: '关注方向', type: 'select', options: promptOptions(compatibilityFocusLabels) },
      { key: 'personA', label: '对象 A', type: 'text', placeholder: '可填姓名、昵称或记录名' },
      { key: 'personB', label: '对象 B', type: 'text', placeholder: '可填姓名、昵称或记录名' },
      { key: 'relation', label: '关系说明', type: 'text', placeholder: '例如：合作、亲密关系、亲友、同事' },
      { key: 'question', label: '具体问题', type: 'textarea', placeholder: '写清楚这次只需要字段对照、核验点还是补充问题。' },
      { key: 'context', label: '背景补充', type: 'textarea', placeholder: '补关系背景和限制条件，避免直接下结论。' },
      { key: 'chartA', label: '对象 A 排盘字段', type: 'textarea', placeholder: '粘贴对象 A 的完整八字/紫微字段。' },
      { key: 'chartB', label: '对象 B 排盘字段', type: 'textarea', placeholder: '粘贴对象 B 的完整八字/紫微字段。' }
    ],
    recordSlots: [
      { key: 'chartA', label: '作为对象 A', indicatorLabel: '对象 A' },
      { key: 'chartB', label: '作为对象 B', indicatorLabel: '对象 B' }
    ],
