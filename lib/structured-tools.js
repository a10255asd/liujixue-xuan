...
import { buildNamePlanRecordRows } from './name-plan-record.js'
...
const buildNameReviewRows = ({ profile, fullName, padded, missingStrokeCount, meaningPreference, avoidNotes, candidateNotes }) => buildRows([
    ...
    sections: [
      ...
      },
      {
        ...
        rows: buildRows([
          ...
          ['六十四卦复核记录', '核对本卦、互卦或变卦的上下卦、卦名和八卦五行。 /tools/hexagrams'],
...
const calculateName = input => {
  ...
  const tianElement = getStrokeElement(tian)
  const renElement = getStrokeElement(ren)
  const diElement = getStrokeElement(di)
  ...
  const sourceNote = cleanPromptInput(input.sourceNote, '未填写来源或笔画口径。')
  const candidateNotes = cleanPromptInput(input.candidateNotes, '未填写候选对比备注。')

  return result({
    title: '姓名方案记录',
    subtitle: `${fullName} · ${profile.label} · 笔画 ${padded.join('-')}`,
    badges: [profile.label, `${tianElement} / ${renElement} / ${diElement}`, `总格${total}${getStrokeElement(total)}`, missingStrokeCount ? `缺${missingStrokeCount}字笔画` : '笔画完整'],
    ...
    sections: [
      ...
      },
      {
        title: '三才字段',
        rows: buildRows([
          ['三才', `${tianElement} / ${renElement} / ${diElement}`],
...
export const structuredTools = {
  ...
  name: {
    ...
    defaultInput: {
      ...
      meaningPreference: '',
      candidateNotes: '备选名、读音测试、重名检索和家人反馈待回填。',
      avoidNotes: ''
    },
    fields: [
      ...
      { key: 'sourceNote', label: '来源/笔画口径', type: 'textarea', placeholder: '可记录姓氏来源、康熙笔画查询口径、人工改动原因。' },
      { key: 'namingGoal', label: '命名目标', type: 'textarea' },
      { key: 'meaningPreference', label: '含义/出处偏好', type: 'textarea', placeholder: '可记录希望靠近或避开的字义、典故、风格。' },
      { key: 'candidateNotes', label: '候选/对比备注', type: 'textarea', placeholder: '可记录备选名、读音测试、重名检索、长辈或客户反馈。' },
