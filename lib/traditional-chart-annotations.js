import { baZiArchiveProfiles } from './bazi-chart.js'
import { ziWeiArchiveProfiles } from './ziwei-chart.js'
...
const baZiArchiveBoundary = '只做出生档案、排盘字段和复核清单，不输出吉凶、建议或人生判断。'
const ziWeiPublicSource = '排盘口径：公历/阳历出生时间，默认按出生地真太阳时、页面选择的子时换日规则、性别和闰月处理方式排盘'
const ziWeiArchiveBoundary = '只做命盘档案、本命盘字段和复核清单，不输出吉凶、建议或人生判断。'
...
const getBaZiArchiveProfile = input => baZiArchiveProfiles[input.archivePurpose] || baZiArchiveProfiles.archive
...
const getZiWeiArchiveProfile = input => ziWeiArchiveProfiles[input.archivePurpose] || ziWeiArchiveProfiles.archive
...
export function buildBaZiReviewRows(chart) {
  const profile = getBaZiArchiveProfile(chart.input)
  ...
  const ziHourNote = chart.input.hour >= 23 || chart.input.hour <= 1
    ? `出生小时靠近子时边界，需保留「${sectLabel}」口径并与相邻时辰对照。`
    : `当前不在子时边界，仍保留「${sectLabel}」口径方便复查。`

  return [
    row('档案用途', `${profile.label}：${profile.review}`),
    row('信息来源', chart.input.birthSource || '未填写信息来源'),
    row('校时线索', chart.input.calibrationNotes || '未填写校时线索'),
    row('时间口径', `${timeModeLabel}；${formatTimeAdjustment(chart)}`),
    row('子时复核', ziHourNote),
    row('起运口径', yunSectLabel),
    row('今日小动作', profile.action),
    row('后续复盘', profile.observe),
    row('输出边界', baZiArchiveBoundary)
  ]
}

export function buildZiWeiReviewRows(chart) {
  const profile = getZiWeiArchiveProfile(chart.input)
  ...
  const mingPosition = `${chart.mingPalace?.heavenlyStem || ''}${chart.mingPalace?.earthlyBranch || ''}` || '未取'
  const bodyPosition = `${chart.bodyPalace?.heavenlyStem || ''}${chart.bodyPalace?.earthlyBranch || ''}` || '未取'
  const ziHourNote = chart.input.hour >= 23 || chart.input.hour <= 1
    ? `出生小时靠近子时边界，需保留「${sectLabel}」口径并与相邻时辰对照。`
    : `当前不在子时边界，仍保留「${sectLabel}」口径方便复查。`
  ...
  return [
    ...
    row('信息来源', chart.input.birthSource || '未填写信息来源'),
    row('校时线索', chart.input.calibrationNotes || '未填写校时线索'),
    row('复核重点', chart.input.focusNotes || '未填写复核重点'),
    row('时间口径', `${timeModeLabel}；${formatTimeAdjustment(chart)}`),
    row('子时复核', ziHourNote),
    row('闰月处理', fixLeapLabel),
    row('宫位定位', `命宫 ${mingPosition}；身宫 ${bodyPosition}；五行局 ${chart.fiveElementsClass}`),
    row('今日小动作', profile.action),
    row('后续复盘', profile.observe),
    row('输出边界', ziWeiArchiveBoundary)
  ]
}
...
export function buildBaZiExportPayload(chart) {
  ...
  const yunSectLabel = getBaZiYunSectLabel(chart.input)
  const timeModeLabel = getBaZiTimeModeLabel(chart.input)
  const archiveProfile = getBaZiArchiveProfile(chart.input)
  ...
  return {
    ...
    imageKind: 'baziFine',
    title: '八字出生档案',
    subtitle: `${formatInputDateTime(chart.input)} 公历/阳历 · ${chart.input.gender} · ${archiveProfile.label}${chart.input.birthPlace ? ` · ${chart.input.birthPlace}` : ''}`,
    filename: `bazi-chart-${formatFilenameDateTime(chart.input)}.png`,
    textFilename: `bazi-chart-${formatFilenameDateTime(chart.input)}.txt`,
    badges: [
      archiveProfile.label,
    ...
    sections: [
      ...
      {
        title: '复核清单',
        rows: buildBaZiReviewRows(chart)
      },
      {
        title: '专业细盘',
    ...
    footer: `${baZiPublicSource}；${baZiArchiveBoundary}`
  }
}
...
export function buildZiWeiExportPayload(chart) {
  ...
  const sectLabel = getZiWeiSectLabel(chart.input)
  const fixLeapLabel = getZiWeiFixLeapLabel(chart.input)
  const archiveProfile = getZiWeiArchiveProfile(chart.input)
  ...
  return {
    ...
    sections: [
      ...
      {
        title: '复核清单',
        rows: buildZiWeiReviewRows(chart)
      },
      {
        title: '十二宫专业字段',
        rows: palaceRows
    ...
    chartSolarText: chart.chartSolarText,
    timeAdjustmentText: formatTimeAdjustment(chart),
    footer: `${ziWeiPublicSource}；${ziWeiArchiveBoundary}`
  }
}
...
export function buildBaZiCopyText(chart) {
  ...
  const reviewLines = buildBaZiReviewRows(chart).map(item => `- ${item.label}：${item.value}`).join('\n')

  return [
    '八字出生档案',
    '',
    `输入：${formatInputDateTime(chart.input)}（公历/阳历），性别：${chart.input.gender}`,
    `档案用途：${archiveProfile.label}`,
    `信息来源：${chart.input.birthSource || '未填写信息来源'}`,
    `校时线索：${chart.input.calibrationNotes || '未填写校时线索'}`,
...
export function buildZiWeiCopyText(chart) {
  ...
  const reviewLines = buildZiWeiReviewRows(chart).map(item => `- ${item.label}：${item.value}`).join('\n')
  ...
  return [
    ...
    `信息来源：${chart.input.birthSource || '未填写信息来源'}`,
    `校时线索：${chart.input.calibrationNotes || '未填写校时线索'}`,
    `复核重点：${chart.input.focusNotes || '未填写复核重点'}`,
    `出生地：${formatBirthPlace(chart.input)}`,
    `时间口径：${timeModeLabel}；${formatTimeAdjustment(chart)}`,
    `排盘用时间：${chart.chartSolarText}`,
    `子时口径：${sectLabel}`,
