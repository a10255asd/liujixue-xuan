...
const drawZiWeiCenterCell = (context, payload, x, y, width, height) => {
  ...
  context.fillStyle = textColor
  context.fillText(payload.title || '紫微命盘档案', x + 24, y + 42)
...
const liuYaoColumns = [
  { key: 'label', title: '爻位', width: 64 },
  { key: 'sixGod', title: '六神', width: 70 },
  { key: 'baseText', title: '本卦纳甲', width: 128 },
  { key: 'lineMark', title: '爻象', width: 104 },
  { key: 'movingMarker', title: '动变', width: 76 },
  { key: 'changedText', title: '变卦纳甲', width: 128 },
  { key: 'role', title: '世应', width: 54 },
  { key: 'hiddenText', title: '伏神/飞神', width: 158 },
  { key: 'relationsText', title: '关系', width: 186 }
]

const drawLiuYaoLineMark = (context, bit, x, y, width, moving = false) => {
  const segmentHeight = 8
  const segmentWidth = Math.min(58, width - 20)
  const startX = x + (width - segmentWidth) / 2
  const startY = y - 4

  context.fillStyle = moving ? accentColor : '#252b27'

  if (bit) {
    drawRoundRect(context, startX, startY, segmentWidth, segmentHeight, 4)
    context.fill()
    return
  }

  const gap = 13
  const halfWidth = (segmentWidth - gap) / 2
  drawRoundRect(context, startX, startY, halfWidth, segmentHeight, 4)
  context.fill()
  drawRoundRect(context, startX + halfWidth + gap, startY, halfWidth, segmentHeight, 4)
  context.fill()
}

const measureLiuYaoMetaPairs = (context, payload, maxWidth) => {
  const gap = 10
  const cellWidth = (maxWidth - gap) / 2

  setFont(context, 660, 18)
  const rows = []
  const metaRows = payload.metaRows || []

  for (let index = 0; index < metaRows.length; index += 2) {
    const cells = metaRows.slice(index, index + 2).map(row => ({
      ...row,
      valueLines: wrapText(context, row.value, cellWidth - 92)
    }))
    const rowHeight = Math.max(46, Math.max(...cells.map(cell => cell.valueLines.length)) * 22 + 24)
    rows.push({ cells, rowHeight })
  }

  return {
    cellWidth,
    gap,
    height: rows.reduce((sum, row) => sum + row.rowHeight, 0) + 18,
    rows
  }
}

const measureLiuYaoTable = (context, payload) => {
  setFont(context, 720, 16)
  const rows = (payload.liuYaoLines || []).map((line, index) => {
    const wrapped = {
      hiddenText: wrapText(context, line.hiddenText, liuYaoColumns.find(column => column.key === 'hiddenText').width - 18),
      relationsText: wrapText(context, line.relationsText, liuYaoColumns.find(column => column.key === 'relationsText').width - 18)
    }
    const rowHeight = Math.max(84, Math.max(wrapped.hiddenText.length, wrapped.relationsText.length) * 21 + 34)
    return { ...line, rowHeight, shaded: index % 2 === 0, wrapped }
  })

  return {
    headerHeight: 46,
    rows,
    width: liuYaoColumns.reduce((sum, column) => sum + column.width, 0),
    height: 46 + rows.reduce((sum, row) => sum + row.rowHeight, 0)
  }
}

const createLiuYaoFinePlan = (context, payload, maxWidth) => {
  setFont(context, 680, 24)
  const subtitleLines = wrapText(context, payload.subtitle, maxWidth)

  setFont(context, 760, 20)
  const badgeRows = measureBadges(context, payload.badges || [], maxWidth)
  const metaPlan = measureLiuYaoMetaPairs(context, payload, maxWidth)
  const tablePlan = measureLiuYaoTable(context, payload)
  ...
  const height = 58 +
    ...
    tablePlan.height +
    30 +
    footerLines.length * 26 +
    watermarkSpace

  return {
    badgeRows,
    footerLines,
    height: Math.max(1180, height),
    metaPlan,
    subtitleLines,
    tablePlan
  }
...
const drawLiuYaoMetaCard = (context, plan, x, y, width) => {
  context.fillStyle = '#f8f6f0'
  context.strokeStyle = ruleColor
  context.lineWidth = 1
  drawRoundRect(context, x, y, width, plan.height, 12)
  context.fill()
  context.stroke()
  ...
  for (const [rowIndex, row] of plan.rows.entries()) {
    ...
    for (const [cellIndex, cell] of row.cells.entries()) {
      const cellX = x + 14 + cellIndex * (plan.cellWidth + plan.gap)
      setFont(context, 760, 16)
      context.fillStyle = mutedColor
      context.fillText(`${cell.label}：`, cellX, cellY + 24)

      setFont(context, 680, 17)
      context.fillStyle = textColor
      const valueX = cellX + 78
      for (const [lineIndex, line] of cell.valueLines.entries()) {
        context.fillText(line, valueX, cellY + 24 + lineIndex * 22)
      }
    }

    rowY += row.rowHeight
    if (rowIndex < plan.rows.length - 1) {
      context.strokeStyle = 'rgba(226, 219, 206, 0.72)'
      context.beginPath()
...
const drawLiuYaoSummaryCards = (context, payload, x, y, width) => {
  const items = [
    ['本卦', payload.hexagramSummary?.main || '-'],
    ['变卦', payload.hexagramSummary?.changed || '-'],
    ['世应', payload.hexagramSummary?.worldResponse || '-'],
    ['动爻', payload.hexagramSummary?.moving || '-']
  ]
  const gap = 10
  ...
  for (const [index, [label, value]] of items.entries()) {
    ...
    context.fillStyle = index === 0 ? '#e7f7f3' : '#fffdf8'
    context.strokeStyle = index === 0 ? 'rgba(13, 148, 136, 0.6)' : ruleColor
    context.lineWidth = index === 0 ? 2 : 1
    drawRoundRect(context, cardX, y, cardWidth, 92, 12)
    context.fill()
    context.stroke()

    setFont(context, 760, 16)
    context.fillStyle = mutedColor
    context.fillText(label, cardX + 14, y + 25)

    setFont(context, 860, 21)
    context.fillStyle = index === 0 ? accentColor : textColor
    const lines = wrapText(context, value, cardWidth - 28).slice(0, 2)
    for (const [lineIndex, line] of lines.entries()) {
      context.fillText(line, cardX + 14, y + 58 + lineIndex * 24)
    }
  }
...
const drawLiuYaoTable = (context, plan, x, y) => {
  context.save()
  drawRoundRect(context, x, y, plan.width, plan.height, 12)
  context.clip()

  context.fillStyle = '#f7f3ea'
  context.fillRect(x, y, plan.width, plan.headerHeight)
  context.strokeStyle = ruleColor
  context.lineWidth = 1

  let columnX = x
  setFont(context, 780, 16)
  context.fillStyle = mutedColor
  for (const column of liuYaoColumns) {
    context.fillText(column.title, columnX + 9, y + 29)
    context.beginPath()
    context.moveTo(columnX, y)
    context.lineTo(columnX, y + plan.height)
    context.stroke()
    columnX += column.width
  }

  let rowY = y + plan.headerHeight
  for (const row of plan.rows) {
    context.fillStyle = row.shaded ? '#fffdf8' : '#f8f6f0'
    context.fillRect(x, rowY, plan.width, row.rowHeight)
    context.strokeStyle = ruleColor
    context.beginPath()
    context.moveTo(x, rowY)
    context.lineTo(x + plan.width, rowY)
    context.stroke()

    columnX = x
    for (const column of liuYaoColumns) {
      const cellX = columnX
      const centerY = rowY + row.rowHeight / 2

      if (column.key === 'lineMark') {
        drawLiuYaoLineMark(context, row.bit, cellX + 12, centerY, column.width - 24, row.moving)
      } else if (column.key === 'baseText' || column.key === 'changedText') {
        setFont(context, 840, 19)
        context.fillStyle = elementTextColors[column.key === 'baseText' ? row.baseElement : row.changedElement] || textColor
        context.fillText(row[column.key], cellX + 9, centerY + 7)
      } else if (column.key === 'hiddenText' || column.key === 'relationsText') {
        setFont(context, 680, 15)
        context.fillStyle = column.key === 'relationsText' ? warmColor : textColor
        const lines = row.wrapped[column.key]
        const startY = centerY - ((lines.length - 1) * 21) / 2 + 5
        for (const [lineIndex, line] of lines.entries()) {
          context.fillText(line, cellX + 9, startY + lineIndex * 21)
        }
      } else {
        setFont(context, column.key === 'movingMarker' && row.moving ? 860 : 720, 17)
        context.fillStyle = column.key === 'movingMarker' && row.moving ? accentColor : textColor
        context.fillText(row[column.key] || '-', cellX + 9, centerY + 6)
      }
...
const drawLiuYaoFinePlan = (context, payload, plan, width, height, padding) => {
  context.fillStyle = '#fffdf8'
  context.fillRect(0, 0, width, height)
  ...
  context.fillStyle = mutedColor
  setFont(context, 680, 24)
  for (const line of plan.subtitleLines) {
  ...
  y += 18
  setFont(context, 760, 20)
  for (const row of plan.badgeRows) {
    let badgeX = padding
    for (const badge of row) {
      context.fillStyle = '#e7f7f3'
      drawRoundRect(context, badgeX, y, badge.width, 34, 17)
      context.fill()
      context.fillStyle = accentColor
      context.fillText(badge.text, badgeX + 14, y + 24)
  ...
  y += 24
  y = drawLiuYaoMetaCard(context, plan.metaPlan, padding, y, maxWidth) + 26
  y = drawLiuYaoSummaryCards(context, payload, padding, y, maxWidth) + 34

  setFont(context, 820, 28)
  context.fillStyle = textColor
  context.fillText('六爻排盘', padding, y + 28)
  y += 42
  y = drawLiuYaoTable(context, plan.tablePlan, padding, y) + 30
...
export async function downloadLiuYaoFineChartImage(payload) {
  if (typeof document === 'undefined') return false
  ...
  const measuringCanvas = document.createElement('canvas')
  const measuringContext = measuringCanvas.getContext('2d')
  const plan = createLiuYaoFinePlan(measuringContext, payload, width - padding * 2)
  const scale = Math.max(2, Math.min(3, window.devicePixelRatio || 2))
  const canvas = document.createElement('canvas')
  canvas.width = width * scale
  canvas.height = plan.height * scale
  ...
  context.scale(scale, scale)
  drawLiuYaoFinePlan(context, payload, plan, width, plan.height, padding)

  const url = canvas.toDataURL('image/png')
  const link = document.createElement('a')
  link.href = url
  link.download = payload.filename || 'liuyao-fine-chart.png'
  document.body.appendChild(link)
  link.click()
  document.body.removeChild(link)
...
export async function downloadChartImage(payload) {
  ...
  if (payload?.imageKind === 'liuYaoFine') return downloadLiuYaoFineChartImage(payload)
  if (typeof document === 'undefined') return false
