...
const storageProbeKey = '__jixueXuanStorageProbe'

let lastMemoryWriteMode = 'unavailable'
...
export const getMemoryStorageStatus = () => {
  if (!canUseWindow()) {
    return {
      mode: 'unavailable',
      label: '无法保存',
      description: '当前运行环境没有浏览器存储，记录不会被写入。'
    }
  }
  ...
  try {
    ...
    if (storage) {
      ...
      return {
        mode: 'local',
        label: '本机浏览器保存',
        description: '记录保存在当前浏览器。更换设备、无痕模式或清理浏览器数据后可能看不到，重要记录建议导出 JSON。'
      }
    }
  } catch {
  }
  ...
  if (getFallbackStore()) {
    return {
      mode: 'session',
      label: '当前页面临时保存',
      description: '浏览器本地存储不可用，记录只保存在当前页面会话中。刷新或关闭页面前请导出 JSON。'
    }
  }
  ...
  return {
    mode: 'unavailable',
    label: '无法保存',
    description: '当前浏览器限制了本地存储，记录无法写入。'
  }
}

export const writeMemory = (key, value) => {
  lastMemoryWriteMode = 'unavailable'
...
export const saveMemoryRecord = input => {
  ...
  return {
    ...
    storageMode: lastMemoryWriteMode
  }
}

export const getMemorySaveFeedback = record => {
  if (!record) {
    return {
      label: '保存失败',
      description: '当前浏览器没有允许写入记录。',
      tone: 'error'
    }
  }

  const temporary = record.storageMode === 'session'
  const updated = record.saveMode === 'updated'

  if (temporary) {
    return {
      label: updated ? '临时更新' : '临时保存',
      description: '已写入当前页面会话；刷新或关闭页面前建议导出 JSON。',
      tone: 'warning'
    }
  }

  return {
    label: updated ? '已更新' : '已保存',
    description: '已保存到当前浏览器的本地记录。',
    tone: 'success'
  }
}
...
export const filterMemoryRecordsForTarget = (records, options = {}) => {
  const sourceRecords = Array.isArray(records) ? records : []
  const query = String(options.query || '').trim().toLowerCase()
  const category = String(options.category || 'all')
  const targetSlug = String(options.targetSlug || '')
  ...
  return sourceRecords.filter(record => {
    ...
    const searchable = `${record.tool || ''} ${record.title || ''} ${record.text || ''}`.toLowerCase()
    const matchesQuery = !query || searchable.includes(query)
    const matchesCategory = category === 'all' ||
      (category === 'recommended' ? Boolean(suggestion) : workflow.category === category)
