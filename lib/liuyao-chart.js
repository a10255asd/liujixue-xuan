...
export function normalizeLiuYaoInput(input = {}) {
  ...
  const method = String(merged.method || defaultLiuYaoInput.method).trim().slice(0, 20) || defaultLiuYaoInput.method
  const lines = normalizeLines(merged.lines)

  return { question, year, month, day, hour, minute, method, lines }
}
...
export function calculateLiuYaoChart(input = {}) {
  ...
  const lineStates = normalized.lines
    ...
    sections: [
      {
        ...
        rows: [
          ...
          { label: '起卦方式', value: chart.input.method },
