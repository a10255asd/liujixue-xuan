...
export const liuYaoFocusProfiles = {
  relation: {
    label: '关系复盘',
    review: '先记录双方位置、沟通事实和现实边界，再人工指定用神和世应关系。',
    action: '先补一条事实清单，避免只用情绪或卦名替代沟通判断。',
    observe: '后续记录对方回应、沟通节点和自身状态变化。'
  },
  decision: {
    label: '决策拆解',
    review: '先把选项、限制、资源和待确认信息拆开，再回到六亲、世应和动爻字段。',
    action: '今天只验证一个关键条件，不用卦象替你直接做决定。',
    observe: '后续记录验证结果、选项变化和是否需要重新起卦。'
  },
  timing: {
    label: '时机复核',
    review: '先固定事项窗口、起卦时间和已知限制，再看月建、日辰、旬空和动爻。',
    action: '把时间窗口拆成最近一步和备选窗口，不直接给应期断语。',
    observe: '后续记录事件是否进入新阶段，以及哪个节点出现实质变化。'
  },
  action: {
    label: '行动计划',
    review: '把盘面字段转成可执行动作、风险点和复盘指标。',
    action: '选一个今天能完成的小动作，并写清楚可观察反馈。',
    observe: '复盘动作是否完成、反馈是否清晰、是否需要换路径。'
  },
  archive: {
    label: '卦例留档',
    review: '以留档为主，保存问题、背景、起卦方式、盘面字段和事实回传。',
    action: '补齐来源、背景和后续事实，方便之后横向比较卦例。',
    observe: '回看时只补事实回传，不倒推确定性结论。'
  }
}

export const liuYaoFocusOptions = Object.entries(liuYaoFocusProfiles).map(([value, profile]) => ({
  value,
  label: profile.label
}))
...
export function normalizeLiuYaoInput(input = {}) {
  ...
  const context = String(merged.context || '').trim().slice(0, 180)
  const focus = liuYaoFocusProfiles[merged.focus] ? merged.focus : defaultLiuYaoInput.focus
  ...
  return {
    ...
    context,
    focus,
    focusLabel: liuYaoFocusProfiles[focus].label,
    year,
    month,
    day,
    hour,
...
export function buildLiuYaoReviewRows(chart) {
  const profile = liuYaoFocusProfiles[chart.input.focus] || liuYaoFocusProfiles.relation
  const movingText = chart.movingLines.length ? chart.movingLines.map(item => `${item}爻`).join('、') : '无动爻'

  return [
    { label: '复盘方向', value: `${profile.label}：${profile.review}` },
    { label: '背景材料', value: chart.input.context || '未填写背景材料。' },
    { label: '世应观察', value: `世爻${chart.hexagram.world} / 应爻${chart.hexagram.response}，先作为角色位置留档。` },
    { label: '动爻观察', value: `${movingText}，只记录变化位置，不直接推出结果。` },
    { label: '用神提醒', value: '后续人工分析前，需要先明确用神、事项类型和流派口径。' },
    { label: '今日小动作', value: profile.action },
    { label: '后续复盘', value: profile.observe },
    { label: '输出边界', value: '只做六爻排盘字段、问题材料和复盘清单，不输出吉凶、应期或结果保证。' }
  ]
}

export function buildLiuYaoCopyText(chart) {
  const reviewRows = buildLiuYaoReviewRows(chart)
  const lines = [
    '六爻问事记录',
    `事项：${chart.input.question}`,
    `关注方向：${chart.input.focusLabel}`,
    `背景材料：${chart.input.context || '未填写背景材料。'}`,
    ...
    '',
    '复盘清单：',
    ...reviewRows.map(row => `${row.label}：${row.value}`),
    '',
    '说明：只输出排盘字段、问题材料和复盘清单，不输出吉凶断语；后续分析需另行指定用神、场景和流派口径。'
  ]
...
export function buildLiuYaoExportPayload(chart) {
  const reviewRows = buildLiuYaoReviewRows(chart)
  const linePayload = [...chart.lines].reverse().map(line => ({
    label: line.label,
    sixGod: line.sixGod,
    baseText: `${line.shortRelative}${line.zhi}${line.element}${line.gan}`,
    baseElement: line.element,
    bit: line.bit,
    moving: line.moving,
    movingMarker: line.moving ? line.movingMarker : '',
    changedText: `${line.changed.shortRelative}${line.changed.zhi}${line.changed.element}${line.changed.gan}`,
    changedElement: line.changed.element,
    changedBit: line.changed.bit,
    role: line.role || '-',
    hiddenText: line.hiddenSpirit
      ? `伏 ${line.hiddenSpirit.shortRelative}${line.hiddenSpirit.zhi}${line.hiddenSpirit.element}${line.hiddenSpirit.gan} / 飞 ${line.flyingSpirit.text}`
      : '-',
    relationsText: line.relations.length ? line.relations.join('、') : '-'
  }))

  return {
    toolCode: 'liuyao',
    imageKind: 'liuYaoFine',
    title: '六爻问事记录',
    subtitle: `${chart.dateText} · ${chart.input.focusLabel} · ${chart.input.question}`,
    filename: `liuyao-chart-${chart.dateText.replace(/\D/g, '').slice(0, 12)}.png`,
    badges: [
      chart.input.focusLabel,
    ...
    metaRows: [
      ...
      { label: '关注方向', value: chart.input.focusLabel },
      { label: '背景材料', value: chart.input.context || '未填写' },
      { label: '时间', value: `${chart.dateText}（${chart.lunarText}）` },
      { label: '卦式', value: chart.input.methodLabel },
      { label: '推导', value: formatMethodDetail(chart.methodDetail) },
      { label: '节气', value: chart.jieQiText },
      { label: '干支', value: `${chart.yearGanZhi}年 ${chart.monthGanZhi}月 ${chart.dayGanZhi}日 ${chart.timeGanZhi}时` },
      { label: '月建/日辰', value: `${chart.monthZhi}月 / ${chart.dayZhi}日` },
    ...
    hexagramSummary: {
      main: `${chart.hexagram.name}（${chart.hexagram.kind}，${chart.hexagram.palace}宫${chart.hexagram.palaceElement}）`,
      changed: chart.changedHexagram.name,
      worldResponse: `世爻${chart.hexagram.world} / 应爻${chart.hexagram.response}`,
      moving: chart.movingLines.length ? chart.movingLines.map(item => `${item}爻`).join('、') : '无'
    },
    liuYaoLines: linePayload,
    sections: [
      {
        title: '基础信息',
        rows: [
    ...
    footer: '排盘口径：纳甲、六亲、世应、六神、伏神飞神和关系标记均为字段输出，不输出吉凶、应期或结果保证。'
  }
}
